...
def boson_ops(Nph):
    #  Creates a bosonic operator for a truncated Fock space (with size Nph)
    data,rows, cols = [], [], []
    ...
    adag = a.conjugate().T
    n = adag @ a
    I = sp.identity(Nph, format="csr", dtype=np.complex128) # sparse matrix
    return a, adag, n, I
...
def electron_ops():
    ...
    Returns number operator, c (annihilation), c_dag (creation), and identity matrix
    """
    # Remember: |0> = [1, 0], |1> = [0, 1]
...
def holstein_2site_matrix(t, omega, g, Nph):
    ...
    # define the displacement Operator (a + a^dag)
    x_ph = a + adag
